...
In any event, for a long time I was using an old-style key sequence (Word 2003 and earlier, IIRC) to insert one of these Equation objects: <kbd>Alt</kbd>-<kbd>N</kbd>-<kbd>E</kbd>-<kbd>I</kbd> (the hyphens here indicate to press each key sequentially, rather than holding each key until the end of the sequence).  To streamline things, I [defined the custom shortcut]({{ site.baseurl }}/KSotD-Custom-Word-Shortcuts/) <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>Q</kbd> to provide an alternative, yet-faster way to insert one.
